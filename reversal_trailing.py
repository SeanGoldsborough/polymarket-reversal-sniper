#!/usr/bin/env python3
"""
REVERSAL-TRAIL — Trailing Stop Reversal Strategy
===================================================
Strategy:
  - BTC + SOL
  - Buy cheap token ($0.05-$0.25) on Coinbase reversal signal
  - WebSocket real-time bid monitoring for trailing stop
  - Trailing stop: activate after +$0.10 move, sell when bid drops $0.08 from peak
  - Hold to resolution if trail never activates
...
PAPER_MODE = os.getenv("REVERSAL_TRAIL_LIVE", "0") != "1"

ASSETS = ["BTC", "SOL"]

os.makedirs("logs", exist_ok=True)
BOT_NAME = "REVERSAL-TRAIL" if not PAPER_MODE else "REVERSAL-TRAIL-PAPER"
LOG_FILE = "logs/reversal_trail_trades.jsonl"
SUMMARY_FILE = "logs/reversal_trail_summary.json"
...
class ExecutionEngine:
    ...
    async def buy_taker(self, token_id, price, size):
        ...
        async with self._lock:
            ...
            if shares < 5:
                shares = 5
            buy_price = snap_price(min(price + 0.01, 0.99))
...
# ── WebSocket Position Monitor (Trailing Stop) ─────────
async def ws_monitor_position(bot, pos):
    """
    Monitor position via Polymarket WebSocket for real-time trailing stop.
    Trail activates after +TRAIL_ACTIVATE from entry.
    Sells when bid drops TRAIL_AMOUNT from peak.
    ...
    try:
        async with websockets.connect(POLY_WS_URL, ping_interval=20, ping_timeout=10,
            ...
            async for raw in ws:
                ...
                # Parse best bid from book update
                bids = None
                ...
                # ── TRAIL ACTIVATION ──
                if not trail_active and (bid - entry) >= TRAIL_ACTIVATE:
                    trail_active = True
                    log_msg(f"[TRAIL] #{pos['id']} ACTIVATED @ ${bid:.2f} (peak ${peak_bid:.2f}, entry ${entry:.2f})")

                # ── TRAIL STOP CHECK ──
                if trail_active and (peak_bid - bid) >= TRAIL_AMOUNT:
                    log_msg(f"[TRAIL-STOP] #{pos['id']} bid ${bid:.2f} dropped ${peak_bid - bid:.2f} from peak ${peak_bid:.2f} — SELLING")
                    if bot.engine and not PAPER_MODE:
                        sold = await bot.engine.emergency_sell(
                            token_id, round(pos["shares"]), f"TRAIL-{pos['id']}"
                        )
                        if sold:
                            log_msg(f"[TRAIL-STOP] #{pos['id']} SOLD via emergency_sell")
                            return {"exit": "TRAIL-STOP", "peak": peak_bid, "exit_price": bid,
                                    "trail_active": True, "bid_history": bid_history[-50:]}
                    else:
                        return {"exit": "TRAIL-STOP", "peak": peak_bid, "exit_price": bid,
                                "trail_active": True, "bid_history": bid_history[-50:]}
                ...
                if now > window_end + 60:
                    log_msg(f"[EXPIRE] #{pos['id']} Window ended, bid=${bid:.2f}, trail={'ON' if trail_active else 'OFF'}")
                    if bid > 0.5:
    ...
    return await http_monitor_position(bot, pos, peak_bid, trail_active, bid_history)


async def http_monitor_position(bot, pos, peak_bid=None, trail_active=False, bid_history=None):
    """HTTP polling fallback for trailing stop monitoring."""
    token_id = pos["token_id"]
    entry = pos["entry_price"]
    if peak_bid is None:
        peak_bid = entry
    ...
    while True:
        ...
        if not trail_active and (bid - entry) >= TRAIL_ACTIVATE:
            trail_active = True
            log_msg(f"[TRAIL-HTTP] #{pos['id']} ACTIVATED @ ${bid:.2f}")

        # Trail stop
        if trail_active and (peak_bid - bid) >= TRAIL_AMOUNT:
            log_msg(f"[TRAIL-STOP] #{pos['id']} bid ${bid:.2f} dropped from peak ${peak_bid:.2f} — SELLING (HTTP)")
            if bot.engine and not PAPER_MODE:
                sold = await bot.engine.emergency_sell(
                    token_id, round(pos["shares"]), f"TRAIL-{pos['id']}"
                )
...
class ReversalTrailBot:
    def __init__(self):
        ...
        self.trail_exits = 0      # Count of trail stop exits

    def init_clob(self):
        if PAPER_MODE:
            log_msg("[CLOB] PAPER MODE — no real orders")
            log_msg("[CLOB] Set REVERSAL_TRAIL_LIVE=1 to enable live trading")
            self.bankroll = 100.0
    ...
    async def run(self):
        log_msg("[TRAIL] Waiting for Coinbase feed...")
        while not self.coinbase.connected or self.coinbase.price == 0:
            await asyncio.sleep(0.5)
        log_msg(f"[TRAIL] Coinbase live: BTC ${self.coinbase.price:,.2f}")
    ...
    async def _close_position(self, pos, result):
        ...
        trail_active = result.get("trail_active", False)

        if exit_type == "TRAIL-STOP":
            # P&L based on actual exit price vs entry
        ...
        trail_str = f" trail={'ON' if trail_active else 'OFF'}"
        log_msg(f"[{mode}] #{pos['id']} {pos['asset']} {pos['side']} {exit_type} | "
                f"P&L ${pnl:+.2f} ({ret_pct:+.0f}%) | peak ${peak_bid:.2f}{trail_str} | "
                f"bank ${self.bankroll:.2f} | {self.wins}W/{self.losses}L ({wr:.0f}%) | held {elapsed:.0f}s")
        ...
        icon = "🟢" if pnl > 0 else "🔴"
        trail_tag = " 📐TRAIL" if exit_type == "TRAIL-STOP" else ""
        asyncio.create_task(send_telegram(
            f"{icon}{trail_tag} <b>{BOT_NAME} #{pos['id']}</b>\n"
            f"[{mode}] {pos['asset']} {pos['side']} @ ${pos['entry_price']:.2f} -> {exit_type}\n"
            f"P&L: ${pnl:+.2f} ({ret_pct:+.0f}%) | Peak: ${peak_bid:.2f}\n"
            f"Trail: {'ON' if trail_active else 'never activated'}\n"
    ...
    def print_status(self):
        ...
        print(f"  Strategy: Reversal + Trailing Stop (WS monitor)")
        print(f"  Trail: activate +${TRAIL_ACTIVATE} | stop -${TRAIL_AMOUNT} from peak")
        ...
        print(f"  Trail exits: {self.trail_exits} | Signals: {self.signals_detected} | Reversals: {self.reversals_detected}")
        if self.position:
            pos = self.position
...
async def main():
    ...
    print("=" * 65)
    print(f"  {BOT_NAME} — Trailing Stop Reversal [{mode}]")
    print("=" * 65)
    print(f"  Buy cheap tokens on Coinbase reversal")
    print(f"  Trail: activate +${TRAIL_ACTIVATE} from entry, stop -${TRAIL_AMOUNT} from peak")
    print(f"  Monitor: WebSocket real-time (HTTP fallback)")
    ...
    if mode == "LIVE":
        ...
        print(f"  Paper mode — set REVERSAL_TRAIL_LIVE=1 to go live")
    print()

    bot = ReversalTrailBot()
    bot.init_clob()
    ...
    asyncio.create_task(send_telegram(
        f"🎯 <b>{BOT_NAME}</b> [{mode}]\n"
        f"Trailing stop reversal (WebSocket monitor)\n"
        f"Trail: +${TRAIL_ACTIVATE} activate / -${TRAIL_AMOUNT} stop\n"
        f"{', '.join(ASSETS)} | {int(BET_PCT*100)}% bet\n"
