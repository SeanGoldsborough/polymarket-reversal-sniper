#!/usr/bin/env python3
"""
Polymarket ENDGAME ALT — BTC 5-Minute Markets
================================================
Fixed entry at $0.81, fixed take profit at $0.94.
...
# ============================================================
# CONFIGURATION — ALT strategy: fixed entry $0.81, exit $0.94
# ============================================================
DRY_RUN = os.getenv("DRY_RUN", "true").lower() == "true"

# Kelly sizing — only kicks in when bankroll > $60
KELLY_FRACTION = float(os.getenv("KELLY_FRACTION_ALT", "0.10"))
STARTING_BANKROLL = float(os.getenv("STARTING_BANKROLL_ALT", "500.0"))
MAX_BET_CAP = float(os.getenv("MAX_BET_CAP_ALT", "2000.0"))
KELLY_MIN_BANKROLL = 60.0  # use minimum bet below this

# ALT fixed prices
ENTRY_PRICE = 0.81   # fixed limit buy price
EXIT_PRICE = 0.94    # fixed limit sell price (take profit)
STOP_LOSS_PRICE = 0.44  # market sell if drops to this

# Signal parameters
BTC_MIN_GAP = float(os.getenv("BTC_MIN_GAP_ALT", "15.0"))
BTC_MAX_GAP = float(os.getenv("BTC_MAX_GAP_ALT", "18.0"))
PRICE_PER_TICK = float(os.getenv("PRICE_PER_TICK_ALT", "10.0"))
COOLDOWN = float(os.getenv("EG_COOLDOWN_ALT", "5.0"))
TIME_EXIT = float(os.getenv("EG_TIME_EXIT_ALT", "30.0"))
...
class EndgameProductionBot:
    STATE_FILE = "logs/endgame_5min_ALT_state.json"
    ...
    def __init__(self):
        ...
        self.log_file = open("logs/prod_endgame_5min_ALT_trades.jsonl", "a")
        self.bankroll_log = open("logs/prod_endgame_5min_ALT_bankroll.csv", "a")
        self.bankroll_log.write("timestamp,bankroll,peak,drawdown_pct\n")
        self.trajectory_log = open("logs/prod_endgame_5min_ALT_trajectories.jsonl", "a")
    ...
    # ------- SIGNAL (ALT: 1 trade per 5-min window) -------

    def check_signal(self, gap: float):
        if abs(gap) < BTC_MIN_GAP or abs(gap) > BTC_MAX_GAP:
            return
        # Time filter (ET): skip 9-11am and 2-3pm based on win rate analysis
        et_hour = (datetime.now(timezone.utc).hour - 4) % 24
        if et_hour in (9, 10, 11, 14, 15):
            return
        # Auto-clear stale trades — if older than 5 min, market resolved
        if self.open_trade and time.time() - self.open_trade.opened_at > 300:
            log_msg(f"[STALE] Clearing expired trade #{self.open_trade.id} to unblock trading")
    ...
    async def open_position(self, direction: str, gap: float):
        ...
        # Read real unified order book — confirm token is in ENDGAME zone
        book = await self.book_reader.get_prices(token_id)
        if not book or book.get("midpoint", 0) < 0.75:
            return

        # ALT: hybrid maker-then-taker. Try maker at bid first, fall back to ask
        ask = book.get("best_ask", 0)
        bid = book.get("best_bid", 0)
        if ask < 0.78 or ask > 0.84:
            return  # ask outside our entry range
        entry_price = snap_price(ask)  # final fallback price
        maker_price = snap_price(bid)  # try this first

        # ALT bet sizing: min 5 shares when bankroll < $60, Kelly when >= $60
        if self.bankroll >= KELLY_MIN_BANKROLL:
            bet_size = min(self.bankroll * KELLY_FRACTION, MAX_BET_CAP)
        else:
            bet_size = 6.0 * entry_price  # minimum: 6 shares (ensures 5+ after dust)

        shares = round(bet_size / entry_price, 2)

        # Buy minimum 6 shares
        if shares < 6.0:
            shares = 6.0
        ...
        if DRY_RUN:
            ...
            # HYBRID: try maker bid first, wait 5s, fall back to taker ask
            maker_order_id = await self._submit_order(token_id, maker_price, shares)
            if maker_order_id:
                log_msg(f"[LIVE]  MAKER BID #{trade.id} {direction} @ ${maker_price:.2f}"
                        f" | {shares:.1f} shares | order={maker_order_id[:12]}...")
                await asyncio.sleep(5)

                # Check if it filled
                token_balance = await self._get_token_balance(token_id)
                if token_balance >= shares * 0.9:  # 90%+ filled
                    trade.order_id = maker_order_id
                    trade.entry_token_price = maker_price
                    trade.cost_usd = shares * maker_price
                    log_msg(f"[LIVE]  MAKER FILLED #{trade.id} @ ${maker_price:.2f}")
                else:
                    # Cancel maker, fall back to taker
                    try:
                        self.clob_client.cancel(maker_order_id)
                    except Exception:
                        pass
                    log_msg(f"[LIVE]  Maker not filled ({token_balance:.2f}/{shares:.1f}) — taking ask")
                    await asyncio.sleep(1)

                    order_id = await self._submit_order(token_id, entry_price, shares)
                    if order_id:
                        trade.order_id = order_id
                        log_msg(f"[LIVE]  TAKER #{trade.id} {direction} @ ${entry_price:.2f}"
                                f" | order={order_id[:12]}...")
                    else:
                        log_msg(f"[LIVE]  ORDER FAILED #{trade.id}")
                        return
            else:
                log_msg(f"[LIVE]  Maker order failed — trying taker")
                order_id = await self._submit_order(token_id, entry_price, shares)
                if order_id:
                    trade.order_id = order_id
                    log_msg(f"[LIVE]  TAKER #{trade.id} {direction} @ ${entry_price:.2f}"
        ...
        # LIVE: place take-profit sell and start stop-loss + 30s confirm monitors
        if not DRY_RUN and trade.order_id:
            asyncio.create_task(self._place_take_profit(trade))
            asyncio.create_task(self._check_stop_loss(trade))
            asyncio.create_task(self._check_30s_confirm(trade))
    ...
    async def _check_stop_loss(self, trade):
        """Monitor real book price and trigger stop loss if bid drops to $0.44.
        Cancels TP order and places a limit sell at $0.40."""
        STOP_LOSS_TRIGGER = 0.44
        STOP_LOSS_SELL = 0.40
        try:
            ...
            while self.open_trade and self.open_trade.id == trade.id:
                ...
                # TP detection: if bid >= TP price, the TP order has filled
                if book and book.get("best_bid", 0.0) >= EXIT_PRICE:
                    actual_balance = await self._get_token_balance(trade.token_id)
                    if actual_balance < 0.01:
                        log_msg(f"[STOP]  #{trade.id} TP filled (bid=${book['best_bid']:.2f}, balance=0)")
                        proceeds = trade.shares * EXIT_PRICE
                        pnl = round(proceeds - trade.cost_usd, 2)
                        self._finalize_exit(trade, EXIT_PRICE, 0.0, pnl, "TAKE_PROFIT")
                ...
                if book and book.get("best_bid", 1.0) <= STOP_LOSS_TRIGGER:
                    ...
                    # Place limit sell at $0.40
                    log_msg(f"[STOP]  Placing stop loss sell: {sell_shares:.2f} shares @ ${STOP_LOSS_SELL:.2f}")
                    sell_order_id = await self._submit_sell_order(
                        trade.token_id, STOP_LOSS_SELL, sell_shares)

                    if sell_order_id:
                        log_msg(f"[STOP]  STOP LOSS ORDER #{trade.id} @ ${STOP_LOSS_SELL:.2f}"
                                f" | {sell_shares:.1f} shares | order={sell_order_id[:12]}...")
                        # Finalize with estimated proceeds
                        proceeds = sell_shares * STOP_LOSS_SELL
                        pnl = round(proceeds - trade.cost_usd, 2)
                        self._finalize_exit(trade, STOP_LOSS_SELL, 0.0, pnl, "STOP_LOSS")
                    else:
                        # Limit sell failed — try market sell
                        log_msg(f"[STOP]  Limit sell failed — trying market sell")
                        sell_order_id = await self._submit_market_sell(
                            trade.token_id, sell_shares)
                        if sell_order_id:
                            proceeds = sell_shares * bid
                            pnl = round(proceeds - trade.cost_usd, 2)
                            self._finalize_exit(trade, bid, 0.0, pnl, "STOP_LOSS_MARKET")
                        else:
                            log_msg(f"[STOP]  Market sell also failed #{trade.id}")
    ...
    def print_status(self):
        ...
        print(f"\n  [{ts()}] {'='*60}")
        print(f"  {mode} | {elapsed/60:.0f}min | BTC ENDGAME 5M ALT (entry=$0.81 exit=$0.94)")
        ...
        et_hour = (datetime.now(timezone.utc).hour - 4) % 24
        in_window = et_hour not in (9, 10, 11, 14, 15)
        status = "ACTIVE" if in_window else "PAUSED (skip hour)"
        print(f"  Trading hours: 12a-9a / 12p-2p / 4p-12a ET — currently {status}")
        print()
    ...
    def print_final(self):
        ...
        print(f"\n  Logs: logs/prod_endgame_5min_ALT_trades.jsonl")
        print(f"        logs/prod_endgame_5min_ALT_bankroll.csv")
        print("=" * 65)
