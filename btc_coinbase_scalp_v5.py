...
class BTCScalpBot:
    ...
    async def _execute_trade(self, tid, direction, entry_price, tp_price, sl_price,
        ...
        tier_shares = [int(fill_shares) // 4 + (1 if i < int(fill_shares) % 4 else 0) for i in range(4)]
        tier_offsets = [0.01, 0.02, 0.03, 0.04]
        tier_prices = [snap_price(fill_price + off) for off in tier_offsets]
        tier_order_ids = []
        tier_filled = [False] * 4
        tier_sold_shares = [0] * 4
        tier_sold_revenue = [0.0] * 4

        log_msg(f"[V5-TIERS] #{tid} Placing 4 TP tiers: " +
                " | ".join([f"{tier_shares[i]}sh@${tier_prices[i]:.2f}" for i in range(4)]))

        for i in range(4):
            if self.client and not PAPER_MODE:
                try:
                    args = OrderArgs(price=tier_prices[i], size=tier_shares[i], side=SELL, token_id=trade_token)
                    signed = self.client.create_order(args)
                    resp = self.client.post_order(signed, OrderType.GTC)
                    tier_order_ids.append(resp.get("orderID", ""))
                except:
                    tier_order_ids.append(None)
            else:
                tier_order_ids.append(f"paper_{i}")
                await asyncio.sleep(PAPER_LATENCY)

        # ── STEP 5: Monitor tiers — check fills, cascade unfilled down ──
        total_sold = 0
        total_revenue = 0.0
        ...
        while total_sold < int(fill_shares) and time.time() < window_end:
            ...
            if now >= window_end - FORCE_EXIT_BEFORE_END:
                unsold = int(fill_shares) - total_sold
                if unsold > 0:
                    log_msg(f"[V5-FORCE] #{tid} Window ending, {unsold}sh unsold")
                    # Cancel all open orders
                    if self.client and not PAPER_MODE:
                        for oid in tier_order_ids:
                            if oid:
                                try:
                        ...
                        try:
                            ...
                            args = OrderArgs(price=sell_p, size=unsold, side=SELL, token_id=trade_token)
                            signed = self.client.create_order(args)
            ...
            # Check each tier for fills
            for i in range(4):
                if tier_filled[i]:
                    continue

                if self.client and not PAPER_MODE and tier_order_ids[i]:
                    try:
                        order = self.client.get_order(tier_order_ids[i])
                        if order:
                            matched = float(order.get("size_matched", 0))
                            if matched >= tier_shares[i]:
                                tier_filled[i] = True
                                tier_sold_shares[i] = tier_shares[i]
                                tier_sold_revenue[i] = tier_shares[i] * tier_prices[i]
                                total_sold += tier_shares[i]
                                total_revenue += tier_sold_revenue[i]
                                log_msg(f"[V5-FILL] #{tid} Tier {i+1}: {tier_shares[i]}sh @ ${tier_prices[i]:.2f} FILLED | {total_sold}/{int(fill_shares)} sold")
                    except:
                        pass
                else:
                    # Paper: tier fills if bid reaches the tier price
                    if _bid >= tier_prices[i]:
                        tier_filled[i] = True
                        tier_sold_shares[i] = tier_shares[i]
                        tier_sold_revenue[i] = tier_shares[i] * tier_prices[i]
                        total_sold += tier_shares[i]
                        total_revenue += tier_sold_revenue[i]
                        log_msg(f"[PAPER-V5-FILL] #{tid} Tier {i+1}: {tier_shares[i]}sh @ ${tier_prices[i]:.2f} FILLED (bid=${_bid:.2f}) | {total_sold}/{int(fill_shares)} sold")

            # CASCADE: If price is moving against us, cascade unfilled tiers down
            # Start cascading when price drops to entry - $0.02 (SL level)
            if _bid <= fill_price - SL_OFFSET and not cascade_active:
                cascade_active = True
                log_msg(f"[V5-CASCADE] #{tid} Bid ${_bid:.2f} <= SL — cascading unfilled tiers down")

            if cascade_active:
                # Find unfilled tiers and try to sell at current bid
                for i in range(3, -1, -1):  # Start from highest unfilled tier
                    if tier_filled[i]:
                        continue
                    # Cancel the original order
                    if self.client and not PAPER_MODE and tier_order_ids[i]:
                        try:
                            self.client.cancel_orders([tier_order_ids[i]])
                        except:
                            pass

                    # Try to sell at current bid (maker)
                    cascade_price = snap_price(_bid)
                    if cascade_price < 0.01:
                        cascade_price = 0.01

                    if self.client and not PAPER_MODE:
                        try:
                            args = OrderArgs(price=cascade_price, size=tier_shares[i], side=SELL, token_id=trade_token)
                            signed = self.client.create_order(args)
                            resp = self.client.post_order(signed, OrderType.GTC)
                            cascade_oid = resp.get("orderID", "")
                            await asyncio.sleep(TIERED_WAIT)
                            order = self.client.get_order(cascade_oid)
                            if order and float(order.get("size_matched", 0)) >= tier_shares[i]:
                                tier_filled[i] = True
                                tier_sold_shares[i] = tier_shares[i]
                                tier_sold_revenue[i] = tier_shares[i] * cascade_price
                                total_sold += tier_shares[i]
                                total_revenue += tier_sold_revenue[i]
                                log_msg(f"[V5-CASCADE] #{tid} Tier {i+1}: {tier_shares[i]}sh @ ${cascade_price:.2f} FILLED | {total_sold}/{int(fill_shares)} sold")
                            else:
                                try:
                                    self.client.cancel_orders([cascade_oid])
                                except:
                                    pass
                        except:
                            pass
                        ...
                        if _bid_now >= cascade_price:
                            tier_filled[i] = True
                            tier_sold_shares[i] = tier_shares[i]
                            tier_sold_revenue[i] = tier_shares[i] * cascade_price
                            total_sold += tier_shares[i]
                            total_revenue += tier_sold_revenue[i]
                            log_msg(f"[PAPER-V5-CASCADE] #{tid} Tier {i+1}: {tier_shares[i]}sh @ ${cascade_price:.2f} FILLED | {total_sold}/{int(fill_shares)} sold")
                        else:
                            # Try one level lower next iteration
                            pass

                    break  # Only try one tier per loop iteration
        ...
        cost = fill_price * int(fill_shares)
        pnl_from_tiers = total_revenue - cost

        filled_tiers = sum(1 for f in tier_filled if f)
        exit_reason = f"V5-{filled_tiers}T"
        if cascade_active:
            exit_reason += "+C"

        log_msg(f"[V5-DONE] #{tid} {total_sold}/{int(fill_shares)}sh sold | avg ${exit_price:.3f} | "
                f"P&L ${pnl_from_tiers:+.2f} | {filled_tiers}/4 tiers filled")
