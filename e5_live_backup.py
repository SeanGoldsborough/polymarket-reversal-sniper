...
class ExecutionEngine:
    ...
    async def check_fill(self, token_id, entry_price, max_wait=20):
        """Poll book to check if maker bid filled."""
    ...
    async def _run_variant(self, key, up_tok, down_tok, question, window_end):
        ...
        if self.engine:
            ...
            if not order:
                ...
                v.unfilled += 1
                return
            # Fill verification: poll up to 20s
            log_msg(f"[{key}] #{tid} Waiting for fill...")
            filled = await self.engine.check_fill(token_id, entry_price, max_wait=20)
            if not filled:
                await self.engine.cancel_order(order["order_id"])
                log_msg(f"[{key}] #{tid} Not filled -- cancelled")
                v.unfilled += 1
                return
            log_msg(f"[{key}] #{tid} FILLED @ ${entry_price:.2f}")
        else:
