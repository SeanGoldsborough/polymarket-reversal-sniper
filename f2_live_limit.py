#!/usr/bin/env python3
"""
F2LiveLimit — BTC 5-Minute Markets
=====================================
T-45 entry on the leading side (limit order at ask + $0.01).
...
STARTING_BANKROLL = float(os.getenv("STARTING_BANKROLL_F2", "100.0"))
...
class EndgameProductionBot:
    STATE_FILE = "logs/f2_live_limit_state.json"
    ...
    def __init__(self):
        ...
        self.log_file = open("logs/f2_live_limit_trades.jsonl", "a")
        self.bankroll_log = open("logs/f2_live_limit_bankroll.csv", "a")
        self.bankroll_log.write("timestamp,bankroll,peak,drawdown_pct\n")
        self.trajectory_log = open("logs/f2_live_limit_trajectories.jsonl", "a")
    ...
    async def open_position(self, direction: str, gap: float):
        ...
        # F2Limit: limit buy at ask + $0.01 (ensures fill, still a limit order)
        ask = book.get("best_ask", 0)
        entry_price = snap_price(ask + 0.01)
        ...
        if DRY_RUN:
            ...
            # F2Limit: limit buy at ask + $0.01
            order_id = await self._submit_order(token_id, entry_price, shares)
            if order_id:
                trade.order_id = order_id
                log_msg(f"[LIVE]  LIMIT BUY #{trade.id} {direction} @ ${entry_price:.2f}"
                        f" (ask+$0.01) | {shares:.1f} shares | order={order_id[:12]}...")
            else:
        ...
        # LIVE: start stop-loss monitor (handles both SL and TP detection)
        # No TP order placed — we hold to resolution ($0.99+ bid → sell)
        # No 30s confirm — the dynamic SL handles early exit
    ...
    def print_status(self):
        ...
        print(f"\n  [{ts()}] {'='*60}")
        print(f"  {mode} | {elapsed/60:.0f}min | BTC F2 LIVE LIMIT (entry=$0.81 exit=$0.94)")
    ...
    def print_final(self):
        ...
        print(f"\n  Logs: logs/f2_live_limit_trades.jsonl")
        print(f"        logs/f2_live_limit_bankroll.csv")
        print("=" * 65)
...
async def run_f2_watcher(bot: EndgameProductionBot):
    ...
    while True:
        try:
            ...
            if up_book.get("midpoint", 0) > down_book.get("midpoint", 0):
                direction = "UP"
            elif down_book.get("midpoint", 0) > up_book.get("midpoint", 0):
                direction = "DOWN"
            else:
                continue  # no clear leader
...
async def main():
    mode = "PAPER" if DRY_RUN else "*** LIVE ***"
    print("=" * 65)
    print(f"  F2 LIVE LIMIT — {mode}")
    print("=" * 65)
    print()
    print(f"  Mode:        {mode}")
    print(f"  Strategy:    F2 — T-{TRIGGER_REMAINING} limit buy (ask+$0.01) on leading side")
