...
TARGET_SHARES = 100         # Ideal trade size (will scale down if book is thin)
...
PAPER_MODE = os.getenv("BTC_SCALP_V3_LIVE", "0") != "1"
POLY_WS_URL = "wss://ws-subscriptions-clob.polymarket.com/ws/market"
CB_WS_URL = "wss://ws-feed.exchange.coinbase.com"

os.makedirs("logs", exist_ok=True)
BOT_NAME = "BTC-SCALP-V3" if not PAPER_MODE else "BTC-SCALP-V3-PAPER"
LOG_FILE = "logs/btc_scalp_v3_trades.jsonl"
SUMMARY_FILE = "logs/btc_scalp_v3_summary.json"
...
async def get_book_depth(token_id, side="asks"):
    """Get order book depth for a token. Returns list of (price, size) tuples sorted best-first."""
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get(f"https://clob.polymarket.com/book?token_id={token_id}",
                ...
                if r.status == 200:
                    ...
                    if side == "asks":
                        return sorted([(float(e["price"]), float(e["size"])) for e in entries], key=lambda x: x[0])
                    else:
                        return sorted([(float(e["price"]), float(e["size"])) for e in entries], key=lambda x: -x[0])
    except:
        pass
...
def calc_dynamic_shares(depth, entry_price, max_slippage=0.01):
    """Calculate how many shares we can buy with acceptable slippage.

    Args:
        depth: list of (price, size) from get_book_depth (asks, sorted low to high)
        entry_price: the price we want to enter at
        max_slippage: maximum acceptable avg price above entry (default $0.01)

    Returns:
        shares: number of shares to trade (between MIN_SHARES and MAX_SHARES)
    """
    if not depth:
        return MIN_SHARES
    ...
    for price, size in depth:
        # Only consider levels at or near our entry price
        if price > entry_price + max_slippage:
            break
    ...
    shares = max(MIN_SHARES, min(int(total_shares), MAX_SHARES))
    # Must be at least 5 (Polymarket minimum)
    shares = max(5, shares)
...
class BTCScalpBot:
    ...
    async def run(self):
        ...
        log_msg(f"[INIT] Shares: {MIN_SHARES}-{MAX_SHARES} (dynamic) | Fill timeout: {FILL_TIMEOUT}s | Entry: ${MIN_ENTRY_PRICE}-${MAX_ENTRY_PRICE} | First {ENTRY_WINDOW_SECONDS}s only")
    ...
    async def _trade_window(self, mkt, window_start, window_end):
        ...
        async def trade_executor():
            while time.time() < window_end:
                ...
                # Dynamic sizing: check book depth and size to available liquidity
                token = up_token if direction == "UP" else down_token
                depth = await get_book_depth(token, side="asks")
                trade_shares = calc_dynamic_shares(depth, entry_price)

                if trade_shares < MIN_SHARES:
                    log_msg(f"[SKIP] {direction} book too thin for entry @ ${entry_price:.2f}")
                    continue
    ...
    async def _execute_trade(self, tid, direction, entry_price, tp_price, sl_price,
        ...
        if self.client and not PAPER_MODE:
            try:
                ...
                args = OrderArgs(price=entry_price, size=num_shares, side=BUY, token_id=token_id)
                signed = self.client.create_order(args)
                resp = self.client.post_order(signed, OrderType.GTC)
                buy_order_id = resp.get("orderID", "")
                log_msg(f"[BUY] #{tid} GTC {num_shares}sh @ ${entry_price:.2f} order={buy_order_id[:10]}...")
            except Exception as e:
            ...
            log_msg(f"[PAPER-BUY] #{tid} GTC {num_shares}sh {direction} @ ${entry_price:.2f} "
    ...
    def _print_status(self):
        ...
        print(f"  CB trigger: >${CB_TRIGGER} | TP: +${TP_OFFSET} | SL: -${SL_OFFSET} | {num_shares}sh", flush=True)
    ...
    def _write_summary(self):
        ...
        summary = {
            ...
            "config": {
                ...
                "shares": f"{MIN_SHARES}-{MAX_SHARES} dynamic", "fill_timeout": FILL_TIMEOUT,
